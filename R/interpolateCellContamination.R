...
#' @param interpolationMethod Method to use to interpolate.  \code{lowess} uses a lowess fit, \code{linear} uses linear interpolation and \code{global} set all cells to have the global contamination value.
...
interpolateCellContamination = function(sc,channelName,interpolationMethod = c('lowess','linear','global'),useGlobal=FALSE,...){
  interpolationMethod = match.arg(interpolationMethod)
  if(useGlobal)
    interpolationMethod='global'
  ...
  if(interpolationMethod=='global' | nrow(sc$rhoGrouped)<4){
