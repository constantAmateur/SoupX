...
## ----set_clustering------------------------------------------------------
data(PBMC_metaData)
sc = setClusters(sc,PBMC_metaData$Cluster)

## ----add_DR--------------------------------------------------------------
sc = setDR(sc,PBMC_metaData[,c('RD1','RD2')])

## ----plot_annot----------------------------------------------------------
library(ggplot2)
mids = aggregate(cbind(RD1,RD2) ~ Annotation,data=PBMC_metaData,FUN=mean)
gg = ggplot(PBMC_metaData,aes(RD1,RD2)) + 
...
## ----plot_IGKC-----------------------------------------------------------
PBMC_metaData$IGKC = sc$toc['IGKC',rownames(PBMC_metaData)]
gg = ggplot(PBMC_metaData,aes(RD1,RD2)) +
