...
Having loaded our data, the first thing to do is to estimate what the expression profile of the soup looks like.  This is done as follows,

```{r}
scl = estimateSoup(scl)
```

Which modifies the `scl` object to add estimates of the soup expression profile to each channel entry and provide a global `scl$soupMatrix` that summarises the expression profile across channels.  In our case we only have one channel, but it is worth thinking about the case of multiple channels to understand why the SoupX data and functions are structured as they are and because many experiments will involve multiple samples.
