...
If you are loading 10X data with a full cellranger output folder, the cellranger clustering will be loaded automatically.  This is not the case with our data, so we have to specify it manually.
...
Probably the most difficult part of using SoupX is accurately estimating the level of background contamination (represented as rho) in each channel.  There are two ways to do this: using the automatic method `autoEstCont`, or manually providing a list of "non expressed genes".  This vignette will demonstrate both methods, but we anticipate that the automatic method will be used in most circumstances.  Before that we will describe the idea that underpins both approaches; identifying genes that are not expressed by some cells in our data and the expression that we see must be due to contamination.
...
It is worth considering simply manually fixing the contamination fraction at a certain value.  This seems like a bad thing to do intuitively, but there are actually good reasons you might want to.  When the contamination fraction is set too high, true expression will be removed from your data.  However, this is done in such a way that the counts that are most specific to a subset of cells (i.e., good marker genes) will be the absolutely last thing to be removed.  Because of this, it can be a sensible thing to set a high contamination fraction for a set of experiments and be confident that the vast majority of the contamination has been removed.
