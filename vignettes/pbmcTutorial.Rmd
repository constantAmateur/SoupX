...
You install this package like any other R package.  The simplest way is to use the [devtools](https://devtools.r-lib.org/) `install_github` function as follows:
...
```{r estimateSoup, eval=TRUE}
sc = SoupChannel(tod,toc,calcSoupProfile=FALSE)
sc = estimateSoup(sc)
```

One reason you may want to do this is if you have some other way of estimating the soup.  For example, if you only have access to the table of counts then you might want to set the soup profile to be the average across all cells which is highly correlated with the contamination.  This can be done by running
...
By default, when the soup is estimated using `estimateSoup` the table of droplets `tod` is dropped from the `SoupChannel` object to reduce the memory requirements.  Generally, we don't need the full table of droplets once we have determined what the soup looks like.

## Adding extra meta data to the SoupChannel object

We have some extra meta data that it is essential we include in our `SoupChannel` object.  In general you can add any meta data by providing a `data.frame` with row names equal to the column names of the `toc` when building the `SoupChannel` object.  To save time, some meta data for the PBMC data has been bundled with SoupX, which we can load in like this,

```{r load_data_with_mDat, eval=TRUE}
data(PBMC)
...
This function will modify the `metaData` table of `sc` object to add a table giving the contamination fraction estimate.  This approach gives a contamination fraction of $\sim 5$%, compared to $\sim 6$% for the automatic method.
...
This procedure depends on [rstan](https://mc-stan.org/users/interfaces/rstan) and uses a bayesian model to share information between cells.  It is much, **much** slower and there is seldom ever any point bothering.  Information about the fit can be found in `fit` slot of the `sc` object (this is true for all methods for estimating the contamination).
...
Clearly the interpretation of which cells are expressing these genes changes quite dramatically when we correct for soup contamination.  I have included plots of _CD4_ and _CD8_ to show that genes that are not highly expressed in the soup are essentially unchanged by the soup correction.
...
Finally, we will delete the data we downloaded.
