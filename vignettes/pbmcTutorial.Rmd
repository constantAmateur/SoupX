...
This will load the 10X data into a `SoupChannel` object.  This is just a list with some special properties, storing all the information associated with a single 10X channel.  A `SoupChannel` object can also be created manually by supplying a table of droplets and a table of counts.  We will take this approach in this vignette to demonstrate "manual" constrution.  A dataset \code{PBMC} is bundled with SoupX, which includes meta data and cellular barcodes, but not the raw table of droplets.  To load all the PBMC data and create a `SoupChannel` object run,
...
tmp = readLines(tmp)
rownames(tod) = do.call(rbind,strsplit(tmp,'\t'))[,1]
